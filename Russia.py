...
class Number_of_questions():
    ...
    def __init__(self,Username,access):
        ...
        self.tittle=customtkinter.CTkLabel(self.app,text=f"Hello {Cuser} Enter the Number of questions you wish to answer max questions max 73 ")
...
class Get_Data():
    ...
    def recive_questions(self):
        ...
        #qt is the question type and qt 3 will mean questions on OCR Russia will be asked
        ...
        get_from_db = f"SELECT QuestionID,Questions FROM questions WHERE Qt=2 ORDER BY RAND() LIMIT {self.value_num_questions} "
    ...
    def getall(self,num_questions):
        ...
        print("-->entred",entred)
        getall = f"SELECT QuestionID FROM questions WHERE Qt=2 "
...
class Display():
    def __init__(self,num_questions,user,uid,acess):
        ...
        self.napp.title("StudyMate-Russia")
    ...
    def redo(self):
        ...
        print(outof)
        mcr.execute("INSERT INTO progress (name_user,UsersID,Date_last_score,Last_score,outof,Subject) VALUES(%s,%s,%s,%s,%s,%s)",(self.user,self.uid,string_today,self.score,self.on_table,"Russia"))
        db.commit()
        self.napp.destroy()
...
class loginpage():
    ...
    def GetLogin(self):
        ...
        values=(Uname,Pass)
        executest=mcr.execute(selects_details,values)
        access=mcr.fetchone()
        print("acesss--->",access)
...
class progress():
    def __init__(self,name,details,acess):
        ...
        self.graph.title("Progress of Russia from")
    ...
    def retrivesdata(self):
        ...
        mcr.execute("""SELECT Last_score FROM progress WHERE Subject=%s AND name_user=%s AND UsersID=%s ORDER BY outof ASC, Last_score ASC, ProgressID ASC""",("Russia",self.name,self.details))
        for score in mcr.fetchall():
            lst_score.extend(score)
        print("lst_score-->",lst_score)
        mcr.execute("""SELECT outof FROM progress WHERE Subject=%s AND name_user=%s AND UsersID=%s ORDER BY outof ASC, Last_score ASC, ProgressID ASC""",("Russia",self.name,self.details))
        ...
        ax.set_ylabel('score')
        ax.set_xlabel('outof')
        ax.set_title(f"Progress of Russia for {self.name} from {earliest} to {most_recent}")
