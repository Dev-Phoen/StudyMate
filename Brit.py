...
class Number_of_questions():
    ...
    def __init__(self,Username,access):
        ...
        self.tittle=customtkinter.CTkLabel(self.app,text=f"Hello {Cuser} Enter the Number of questions you wish to answer max questions max 92 ")
...
class Get_Data():
    ...
    def recive_questions(self):
        ...
        #qt is the question type and qt 1 will mean questions on OCR history Britain will be asked
        ...
        get_from_db = f"SELECT QuestionID,Questions FROM questions WHERE Qt=1 ORDER BY RAND() LIMIT {self.value_num_questions} "
    ...
    def getall(self,num_questions):
        ...
        print("-->entred",entred)
        getall = f"SELECT QuestionID FROM questions WHERE Qt=1 "
...
class Display():
    def __init__(self,num_questions,user,uid,acess):
        ...
        self.napp.title("StudyMate-Britain")
    ...
    def redo(self):
        ...
        print(outof)
        mcr.execute("INSERT INTO progress (name_user,UsersID,Date_last_score,Last_score,outof,Subject) VALUES(%s,%s,%s,%s,%s,%s)",(self.user,self.uid,string_today,
                                                                                                                                   self.score,self.on_table,"Britan"))
        db.commit()
        self.napp.destroy()
    ...
    def Ends(self):
        ...
        outof=f"{self.score}/{self.on_table}"
        print(outof)
        mcr.execute("INSERT INTO progress (name_user,UsersID,Date_last_score,Last_score,outof,Subject) VALUES(%s,%s,%s,%s,%s,%s)",(self.user,self.uid,string_today,self.score,self.on_table,"Britan"))
        db.commit()
        self.napp.destroy()
    ...
    def create_buttons(self):
        ...
        for i in range(1, self.num_of_buttons + 1):
            ...
            """https://chat.openai.com/c/d188cf59-5535-4b61-ab47-a6524960286d chatGPT/Open Ai was used
                on 12.12.23 to help make the veriables below for the text to go in the buttons """
...
class create_user():
    def __init__(self):
        ...
        self.create_password=customtkinter.CTkEntry(self.create_page,placeholder_text="create password")
...
class loginpage():
    ...
    #used the video as a tutorial on how to make the log in class on
    #3/02/24 in which i used a similar format for the SQL statement and followed the video to make the validation if the username or password is correct
    def __init__(self):
    ...
    def reset_login_page(self):
        ...
        self.newpass=customtkinter.CTkEntry(self.Rapp,placeholder_text="new password")
        self.confirm=customtkinter.CTkEntry(self.Rapp,placeholder_text="confirm password")
...
class progress():
    def __init__(self,name,details,acess):
        ...
        # self.graph.geometry("800x900")
        # self.graph.state("zoomed")
        self.graph.title("Progress of Britain from")
    ...
    def retrivesdata(self):
        # label=customtkinter.CTkLabel(self.graph,"Progress of History britain for",str(self.name))
        # label.grid()
        lst_score=[]
        lst_out_of=[]
        #https://stackoverflow.com/questions/14279849/how-to-remove-time-from-datetime taken on 11/03/24
        mcr.execute("""SELECT Last_score FROM progress WHERE Subject=%s AND name_user=%s AND UsersID=%s ORDER BY outof ASC, Last_score ASC, ProgressID ASC""",("Britan",self.name,
                                                                                                                                                               self.details))
        for score in mcr.fetchall():
            lst_score.extend(score)
        print("lst_score-->",lst_score)
        mcr.execute("""SELECT outof FROM progress WHERE Subject=%s AND name_user=%s AND UsersID=%s ORDER BY outof ASC, Last_score ASC, ProgressID ASC""",("Britan",self.name,
                                                                                                                                                          self.details))
        ...
        prog.title("Progress of History Britain")
        ...
        ax.set_ylabel('score')
        ax.set_xlabel('outof')
        ax.set_title(f"Progress of History Britan for {self.name} from {earliest} to {most_recent}")
